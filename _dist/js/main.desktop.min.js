...
define('../../../_base/js/scorm/initScorm',[], function() {
	'use strict';
	var initScorm = function() {
		var config = Player.Config.general.scorm,
			wrapper = Player.Scorm.Wrapper,
			queryScorm = Player.Helpers.getQueryVariable('scorm'),
			jsonScorm = Player.json.course.scorm,
			scorm = queryScorm === 'false' || jsonScorm === false ? false : true;
		...
		var lessonStatus = Player.Scorm.getScormValue('cmi.core.lesson_status'),
			score = Player.Scorm.getScormValue('cmi.core.score.raw'),
			location = Player.Scorm.getScormValue('cmi.core.lesson_location'),
			previous_score = Player.Scorm.vars['cmi.suspend_data'].previousScore,
			sco_type = Player.json.sco_type,
			sco_prop = sco_type && sco_type.properties && sco_type.properties.type;

		if(($.inArray('atividade', sco_prop || []) >= 0) && lessonStatus == "completed"){			
			Player.Scorm.vars['cmi.suspend_data'].previousScore = Math.max(previous_score, score) || score;
			Player.Scorm.vars['cmi.suspend_data'].atividade = {};
			Player.Scorm.vars['cmi.core.lesson_location'] = '0';
			// Player.Scorm.vars['cmi.core.lesson_status'] = 'incomplete';
		}

		Player.Scorm.vars['cmi.core.score.raw'] = Math.max(previous_score, score) || score;


		//Criado apenas para executar a função de salvarScorm para usuário do 
		//Firefox; Necessário utilizar um alerta para poder salvar todas as
		//informações do cliente.

		var isFirefox = typeof InstallTrigger !== 'undefined';

		var alertMsg = function(){
			alert('O seu progresso no treinamento foi salvo. Clique em OK para continuar.');
		}

		$(window).bind({
			'beforeunload.initscorm': function() {
				Player.Scorm.saveScormData(true);

				$(this).unbind('unload.initscorm');
			...
			'unload.initscorm': function() {
				Player.Scorm.saveScormData(true);

				$(this).unbind('beforeunload.initscorm');
...
define('../../../_base/js/scorm/saveScormData',[], function() {
    ...
    var saveScormData = function(finaliza) {
        var scormWrapper = Player.Scorm.Wrapper,
            ...
            score = scormVars['cmi.core.score.raw'],
            previous_score = scormVars['cmi.suspend_data'].previousScore;

        console.log(scormVars);

        // MANTEM A NOTA MÁXIMA GRAVADA
        scormVars['cmi.core.score.raw'] = Math.max(score, previous_score) || score;
...
console.log('foo', scormWrapper, '-----', scormVars);
...
define('text!resources/multimedia/template.html',[],function () { return '<div id="{{_id}}" class="{{class}} {{^src}}noContent{{/src}}" style="{{style}}" >\n\t<iframe src="{{#src}} {{src}} {{/src}}" width="100%" height="100%" style="border: 0px;">\n\t\t<p>Your browser does not support iframes.</p>\n\t</iframe>\n\t\t\t\t\t\n\t<div class="bgMultimediaIEfix"></div>\n\t<span class="glyphicon glyphicon-edit"></span>\n</div>\n';});
...
define('resources/texto/editor',[], function() {
...
define('../../../../../player/_base/js/functions/findResource',[
	'jquery'
	], function($) {
	...
	var findResource = {
		...
		add: function(resource) {
			var template = resource.data.template,
				rectype = this.resources[template];
			...
			rectype.push(resource);
		},
		
		find: function(resource_type) {
	...
	var buildContent = function() {
		...
		return {
			buildMain: function(json) {
				...
				/**
				 * feature para remoção de slides com atividades
				 * baseada na feature solicitada: https://docs.google.com/document/d/14re1RCozvG7ILAw_cUAQsifSU3xNw_jtmNL8tcQfGWk/edit#
				 
				 * - "No player por final termos um processo onde antes mesmo de disparar a construção do conteudo criaremos um método
				 * que vai filtrar todos os slides que tiverem recursos de de atividade, escolhendo de todos eles um numero X
				 * de questões e exibindo para o usuário final"
				 
				 * - "Uma vez sorteada as questões devemos guardar no SCORM as questões que foram sorteadas para ele e em caso de
				 * retorno as mesmas devem ser exibidas. A nota final deve ser compilada considerando a quantidade de atividades depois do filtro"
				*/
				...
				if (json.randomizeAtividades > 0) {
					var slides_with_ativ = [],
						randomized = Player.Scorm.getScormValue('cmi.suspend_data', 'randomizeAtividades') || [],
						result = [];

					// get slides with atividade
					$.each(json.slides, function(i, slide) {
						$.each(slide.resources, function(j, resource) {
							if (resource.template !== 'atividade') return;
							slides_with_ativ.push(i);
							return false;
						});
					});

					// if was saved into scorm
					if (!randomized.length) {
						// randomize
						randomized = shuffle($.merge([], slides_with_ativ));
						// save into scorm
						Player.Scorm.setScormValue('cmi.suspend_data', 'randomizeAtividades', randomized);
					}
					
					// slice randomized array
					randomized = randomized.slice(0, json.randomizeAtividades);

					// remove non used slides
					// mantém a mesma ordem que foi definido no editor
					/*var result = $.map(json.slides, function(slide, i) {
						// slide atual nao tem atividade dentro dele, por isso retornamos ele pois queremos ele no json
						if ($.inArray(i, slides_with_ativ) == -1) return slide;
						// slide atual é um dos escolhidos para ficar no json. retornamos slide
						if ($.inArray(i, randomized) >= 0) return slide;
						// caindo aqui significa que o slide contem atividade, porém não foi escolhido para permanecer no json
						return;
					});*/
					
					/**
					 * processa os slides, alterando a ordem deles
					 */
					(function() {
						var _randomized = $.merge([], randomized);

						$.each(json.slides, function(i, slide) {
							if ($.inArray(i, slides_with_ativ) == -1) {
								// slide nao tem atividade - adiciona ao result
								result.push(slide);
								return;
							}
							...
							var found_index = $.inArray(i, randomized);
							if (found_index == -1) return;  // slide nao foi escolhido para permanecer no json

							var index = _randomized.shift();
							result.push(json.slides[index]);
						});
					})();

					// substitui slides pelo resultado
					json.slides = result;
				}
				/**
				 * FIM FEATURE REMOÇÃO DE SLIDES COM ATIVIDADES
				 */

				console.log('buildContent for main------', json, Player.Config.selectors.content);
...
define('../../../../../player/_base/js/components/modal',[
	...
	var modal = function(params, $el) {
		...
		this.add = function(obj, params, config) {
			...
			this.centerModals = function() {
				var me = this,
					$elements = me.$modal.find('.modal_item:visible'),
					w = $(window),
					wsize = {
						width: w.width(),
						height: w.height()
				...
				$.each($elements, function(i, element) {
					var $element = $(element);
					$element.css({
						top: Math.max(0, (wsize.height - $element.height()) / 2),
						left: Math.max(0, (wsize.width - $element.width()) / 2)
					});
				});
			};
	}
		...
		for (var i = scormVars.length - 1; i >= 0; i--) {
			if (scormVars[i]) {
				...
				if (scormVars[i] === "cmi.suspend_data") {
					...
					// bug no LMS da BD. wrapper do scorm esta gravando o json com slashes, mas na hora de ler,
					// nao está retirando esses slashes, dando erro de parse do JSON. esse é o motivo desse fix
					// (removendo slashes do suspend_data antes de dar parse)
					Player.Scorm.vars[scormVars[i]] = {};

					value = value.replace(/\\/g, '');
					if (Player.Helpers.isJSON(value)) {
						Player.Scorm.vars[scormVars[i]] = jQuery.parseJSON(value);
					}
				} else {
...
define('../../../../../player/_base/js/helpers/resourceExtend',['jquery'], function($) {
	...
	var resourceExtend = function(resource) {
		...
		this.init = function() {
			...
			Player.Functions.findResource.add(resource);
...
define('text!../../../../_baseClient/_base/js/desktop/slides/slideCapa/template.html',[],function () { return '<div style="{{style}}; background-image:{{background}}" id="{{_id}}" class="{{class}}"></div>';});
...
define('text!../../../../_baseClient/_base/js/desktop/slides/slideFinal/template.html',[],function () { return '<div style="{{style}}; background-image:{{background}}" id="{{_id}}" class="{{class}}"></div>';});

define('../../../../_baseClient/_base/js/desktop/slides/slideFinal/player',[
	'jquery'
], function($) {

	var slideFinalPlayer = function() {
		this.init = function() {
			var me = this,
				$msgNotaFinal = $('<b>Você acertou <span class="notaFinalUsuario"></span>% das questões.</b>').hide(),
				$notaFinal = $msgNotaFinal.find('.notaFinalUsuario');

			Player.Elements.$content.on({
				contentReady: function(e) {
					me.$el.find('.textoCaixaDestaque').append($msgNotaFinal);

					if (me.hasAtividade()) {
						$notaFinal.html(me.getNotaFinal());
						$msgNotaFinal.show();
					}
				},

				atividadeRespondida: function() {
					$notaFinal.html(me.getNotaFinal());
				}
			});
		}
	...
	slideFinalPlayer.prototype.hasAtividade = function() {
		var me = this,
			atividades_com_nota = $.grep(Player.Functions.findResource.find('atividade'), function(atividade) {
				return atividade.data.nota.subType == 'comNota';
			}),
			has_atividade = !!atividades_com_nota.length;

		// sobrescreve o método prototipado para que retorne diretamente true ou false,
		// pois nao precisamos iteragir em todas atividades com nota novamente
		slideFinalPlayer.prototype.hasAtividade = function() {
			return has_atividade;
		}

		return has_atividade;
	}

	slideFinalPlayer.prototype.getNotaFinal = function() {
		return (Player.Scorm.getScormValue('cmi.core.score.raw') || 0);
	}

	return slideFinalPlayer;
});


define('text!../../../../_baseClient/_base/js/desktop/resources/caixaDestaque/template.html',[],function () { return '<div id="{{_id}}" class="{{class}} {{type}}" style="{{style}}">\n\t<div class="iconDestaque"></div>\n\t<div class="tituloCaixaDestaque">\n\t\t<h2 id="cdTitle">{{&title}}</h2>\n\t</div>\n\t<div class="textoCaixaDestaque">\n\t\t<div class="tagP" id="cdContent">{{&content}}</div>\n\t\t<div class="tagP" id="cdContentPrograme">{{&contentPrograme}}</div>\n\t</div>\n</div>';});

/*
	Não importar jquery pois compilação entrega para ele. Explicar melhor na hora de documentar
*/

define('../../../../_baseClient/_base/js/desktop/resources/caixaDestaque/player',[], function() {
...
define('../../../../_baseClient/_base/js/desktop/resources/imagem/player',[
	"zoom"
], function() {
	...
	var imagem = function(template, data) {
		this.init = function() {
			...
			*/
			...
			if(zoomImg){
				this.$el.find('.trow:first .tcell')
				.zoom({
					on:'click', 
					magnify: 1.5,
					onZoomIn: function(){
						resource.$el.toggleClass('zoomActive');
					},
					onZoomOut: function(){
						resource.$el.toggleClass('zoomActive');
					}
...
define('text!../../../../_baseClient/_base/js/desktop/resources/botaoPP/template.html',[],function () { return '<div id="{{_id}}" class="{{class}} {{#type}}{{type}}{{/type}}" style="{{style}}">\n\t<div class="plus"></div>\n\t<div class="btnPPLeft">\t\n\t\t<div class="iconP3 iconP3-modal"></div>\n\t</div>\n\t<div class="btnPPRight"> \n\t\t<div class="tagP">{{&label}}</span>\n\t</div>\n</div>\n';});
...
define('../../../../_baseClient/_base/js/desktop/resources/botaoPP/player',[], function() {
			...
			recurso.$el
				...
				.on({
					click: function(e) {
						...
						recurso.checkUnlockPlayer();
					}
				});
		};

		this.checkUnlockPlayer = function() {
			if (!me.data.lock || me.$el.not('.visited').length) return;
...
define('text!../../../../_baseClient/_base/js/desktop/resources/texto/template.html',[],function () { return '<div id="{{_id}}" class="{{class}}{{#normal.active}} normal {{normal.subType}}{{/normal.active}}{{#balao.active}} balao {{balao.subType}}{{/balao.active}}{{#titulo.active}} titulo {{titulo.subType}}{{/titulo.active}}" style="{{style}}">\n\t<div class="pontaBalao"></div>\n\t<div class="tagP">{{&content}}</div>\n</div>';});

define('../../../../_baseClient/_base/js/desktop/resources/texto/player',['jquery'], function($) {
		...
		this.init = function() {
			...
			*/
			...
			var recurso = this,
				...
				$alternativas = this.$el.find('.alternativa_atividade li'),
			...
			Player.Elements.$content.on({
				scormReady: function() {
					var id_atividade = recurso.$el.attr('id'),
						atvs_scorm = Player.Scorm.getScormValue('cmi.suspend_data').atividade || {},
						feedback_modal_bkp = feedBackModal;

					$.extend(true, recurso.scorm, atvs_scorm[id_atividade]);

					if (!recurso.isAnswered()) return;

					// atividade já respondida

					feedBackModal = false; // apenas para não mostrar as janelas modais de feedback

					// pre-seleciona as opções gravadas no scorm
					$.each(recurso.scorm.respostas, function(i, resposta) {
						$chkOrRadio.filter('[value="' + resposta + '"]').prop('checked', true);
					});
					recurso.enviaResposta();

					// restaura o valor de feedBackModal
					feedBackModal = feedback_modal_bkp;
				},
				...
				contentReady: function(e) {
					...
					// randomize options
					if (recurso.data.sortOptions === true) {
						$alternativas.randomize();
					}
				}
			});
			...
			var feedback_certo = Player.Elements.generalModal.add(recurso, Player.Config.general.modal, {
				...
				actions: {
					"click .btnFechar": function() {
						Player.Elements.generalModal.hide();
					}
				},
				content: "#modalContent"
			});
			...
			var feedback_errado = Player.Elements.generalModal.add(recurso, Player.Config.general.modal, {
				...
				actions: {
					"click .btnFechar": function() {
						Player.Elements.generalModal.hide();
						// Player.Elements.generalModal.hide(feedback_errado); //retirado, pois ie não fechava modal
					}
				},
				content: "#modalContent"
			});
			...
			this.isAnswered = function() {
				return (recurso.scorm.respostas.length > 0);
			}
			...
			this.enviaResposta = function() {
				var id_atividade = $(this).parent().attr('id'),
					checked_options = recurso.$el.find(".chkOrRadio:checked"),
					answers = $.map(checked_options, function(option) {
						return $(option).val();
					}),
					atvs_scorm = Player.Scorm.getScormValue('cmi.suspend_data').atividade,
					atv_scorm = {},
				...
				// prepare to save into scorm
				recurso.scorm.respostas = answers;
				atv_scorm[id_atividade] = recurso.scorm;

				// checaResposta() e calculaMedia() devem vir DEPOIS de ter sido setada a variavel `recurso.scorm.respostas`
				correct_answer = recurso.checaResposta();
				average_score = recurso.calculaMedia();

				// save into scorm
				Player.Scorm.setScormValue('cmi.suspend_data', 'atividade', $.extend(true, {}, atvs_scorm, atv_scorm));

				// save average score
				Player.Scorm.setScormValue('cmi.core.score.raw', average_score);

				// show feedback message
				recurso.showFeedBack(correct_answer);

				Player.Elements.$content.trigger('atividadeRespondida');
			}

			/**
			 * retorna true quando as respostas escolhidas baterem com o gabarito
			 * @return {BOOLEAN} true atividade respondida corretamente
			 */
			this.checaResposta = function() {
				var checked_options = recurso.$el.find(".chkOrRadio:checked"),
					answers = $.map(checked_options, function(option) {
						return $(option).val();
					});

				return answers.compare(gabarito);
			}

			this.showFeedBack = function(correct) {
				var feedBackVisual = recurso.$el.find(".chkOrRadio");

				if (feedBackCircundado) {
					// assinala as opções corretas e erradas
					$.each(feedBackVisual, function(i, option) {
						var option = $(option),
							correct_option = ($.inArray(option.val(), gabarito) >= 0);

						option.parents('li:first')
							.toggleClass('certo', correct_option)
							.toggleClass('errado', !correct && !correct_option);
					});
				}

				// se for o caso, mostra o modal
				if (feedBackModal) {
					Player.Elements.generalModal.show(recurso.data.id + (correct ? 'feedbackCerto' : 'feedbackErrado'));
				}

			}

			this.calculaMedia = function() {
				var atividades = Player.Functions.findResource.find('atividade'),
					// somente atividades "comNota"
					atividades_com_nota = $.grep(atividades, function(atividade) {
						return (atividade.data.nota.subType == 'comNota');
					}),
					// somente as atividades comNota respondidas corretamente
					atividades_corretas = $.grep(atividades_com_nota, function(atividade) {
						return atividade.isAnswered() && atividade.checaResposta();
					}),
					total_com_nota = atividades_com_nota.length,
					media = !total_com_nota ? 0 : (atividades_corretas.length / total_com_nota) * 100;
...
define('../../../../_baseClient/_base/js/desktop/resources/audio/player',['jquery'], function($) {
	...
	var audio = function(template, data) {
		...
		this['in'] = function() {
			...
			if (!self.data.autoplay || !self.data.src || !self.data.type) return;
		...
		this.out = function() {
			...
			self.player = self.renderAudiojs();
			self.bindAudioEvents();
		},
...
define('text!../../../../_baseClient/_base/js/desktop/resources/multimedia/template.html',[],function () { return '<div id="{{_id}}" class="{{class}} {{^src}}noContent{{/src}}" style="{{style}}" >\n\t<iframe src="{{#src}} {{src}} {{/src}}" width="100%" height="100%" style="border: 0px;">\n\t\t<p>Your browser does not support iframes.</p>\n\t</iframe>\n\t\t\t\t\t\n\t<div class="bgMultimediaIEfix"></div>\n\t<span class="glyphicon glyphicon-edit"></span>\n</div>\n';});
...
define('../../../../_baseClient/_base/js/desktop/resources/tabs/player',[], function() {
	var Tabs = function() {
			...
			Player.Elements.$content.on({
				contentReady: function() {
					...
					var resource_id = me.data._id,
						scorm = Player.Scorm.getScormValue('cmi.suspend_data').tabs || {},
						resource_scorm = scorm[resource_id] || {};

					if (resource_scorm.active === undefined) return;
					
					me.elems.$theTab.tabs('option', 'active', resource_scorm.active);
				}
		...
		this.checkUnlockPlayer = function() {
			...
			if (!me.data.lock || tabs_lis.not('.visited').length) return;
		...
		this.renderTabsContent = function() {
			var me = this,
				...
				tabs_lis = me.$el.find('.tabs-nav li'),
				theTab = me.$el.find('.theTab'),
				scorm = Player.Scorm.getScormValue('cmi.suspend_data').tabs || {},
				resource_id = me.data._id,
				resource_scorm = scorm[resource_id] || {},
				current_active = me.data.active,
				new_suspend_data;
			...
			theTab
				...
				.on('tabsactivate.setVisited', function(event, ui) {
					...
					var i = tabs_lis.index(ui.newTab),
						suspend_data = Player.Scorm.getScormValue('cmi.suspend_data').tabs,
						resource_scorm = {},
						new_suspend_data,
						tab = {};
					...
					Player.Scorm.setScormValue('cmi.suspend_data', 'tabs', new_suspend_data);
			...
			$.each(me.data.tabs, function(i, tab) {
				...
				var slide = Player.Functions.buildContent().buildSlide(tab.dynamicContent.data),
					visited = (resource_scorm.tabs && resource_scorm.tabs[i] && resource_scorm.tabs[i].visited) || current_active == i || false;

				tabs_content.eq(i).prepend(slide.$el);

				// marca como visitado o tab (caso tenha sido gravado no SCORM)
...
define("jqueryUIWidget", ["jquery","jqueryUICore"], function(){});
...
define('../../../../_baseClient/_base/js/desktop/resources/tabs/generalInit',[
	"jquery",
	"jqueryUITabs"
...
define('text!../../../../_baseClient/_base/js/desktop/resources/personagem/template.html',[],function () { return '<div class="{{class}} {{#flip}}flip{{/flip}}" id="{{_id}}" style="{{style}}">\n\t<div id="imageWrapper" class="{{^src}}noContent{{/src}}">\n\t\t<img src="{{#src}} {{src}} {{/src}}"/>\n\t\t<span class="glyphicon glyphicon-user"></span>\n\t</div>\n</div>';});

define('../../../../_baseClient/_base/js/desktop/resources/personagem/player',['jquery'], function($) {

	var personagem = function(template, data) {
		this.init = function() {
			$.extend(true, this, new Player.Helpers.resourceExtend(this, arguments));
		}

		this['in'] = function(slide) {
			console.log('caixa de personagem slider IN executed on slide #%d', slide);
		}

		this.out = function(slide) {
			console.log('caixa de personagem slider OUT executed on slide #%d', slide);
		}
...
define('../../../../_baseClient/_base/js/desktop/resources/botaoRD/player',['jquery'], function($) {
	...
	var botaoRD = function(template, data) {
		...
		this.init = function() {
			...
			var recurso = this,
				current_slide = Player.Tree.main.getCurrentSlide();
...
define('player',[
	...
	var jsonUrl = Player.Helpers.getQueryVariable('aula'),
		inlineJSON = window.inlineJSON,
		initializeSCO = function(data){
			Player.json = data;
	...
	if(jsonUrl){
		var url = jsonUrl.indexOf('?') === -1 ? jsonUrl + '?' + new Date().getTime() : jsonUrl + '&' + new Date().getTime(),
		...
		initializeSCO(inlineJSON);
	}else{
		$('body').append('<h1>Dados da aula não encontrados</h1>');
	}
});
require.config({
